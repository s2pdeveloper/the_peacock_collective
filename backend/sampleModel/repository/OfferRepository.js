...
exports.getAndCountAll = async (query) => {
  const res = await Offer.aggregate(query);
  return {
    data: res[0].data,
...
exports.findByCondition = async (conditions, options) =>
  await Offer.findOne(conditions, options);

exports.findAll = async (conditions, options) =>
  await Offer.find(conditions, options);
...
exports.patchStatus = async (existingData, status) => {
  if (status === defaultStatus.DELETED) {
    ...
    return {
      message: apiSuccessStrings.DELETED('Offer'),
    };
  } else {
  ...
  return {
    message: apiSuccessStrings.STATUS_CHANGE('Offer'),
  };
};
