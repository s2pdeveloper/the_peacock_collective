...
exports.getAndCountAll = async (query) => {
  const res = await Tax.aggregate(query);
  return {
    data: res[0].data,
...
exports.findByCondition = async (conditions, options) =>
  await Tax.findOne(conditions, options);

exports.findAll = async (conditions, options) =>
  await Tax.find(conditions, options);
...
exports.patchStatus = async (existingData, status) => {
  if (status === defaultStatus.DELETED) {
    ...
    return {
      message: apiSuccessStrings.DELETED('Tax'),
    };
  } else {
  ...
  return {
    message: apiSuccessStrings.STATUS_CHANGE('Tax'),
  };
};
